...
public class PlayerCombat : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (combatEnemy != null && !resetCamera)
        {
            enemyStats = combatEnemy.GetComponent<DataMemory>();
            enemyCombat = combatEnemy.GetComponent<EnemyCombat>();
            combat = true;
            ProcessCamera();
        ...
        if(scene == 3)
            cameraAdjust(1f, 1.7f, -1f, 10, -20, 0);
    ...
    public void Attack()
    {
        ...
        enemyStats.hurt.Play();
        anim.Play("Attack");
        if (enemyCombat.anim != null)
            enemyCombat.anim.Play("Hurt");
        playerTurn = false;
    }
    ...
    public void ReduceStat()
    {
        ...
        print("Enemy has " + (int)enemyStats.health + " remaining health");
        enemyStats.debuff.Play();
        anim.Play("Buff");
        if (enemyCombat.anim != null)
            enemyCombat.anim.Play("Debuff");
        playerTurn = false;
    }
    ...
    public void SpecialAttack()
    {
        ...
        float damage = Random.Range(65, 90);
    ...
    public void BattleStart()
    {
        ...
        combatEnemy.GetComponent<EnemyCombat>().player = gameObject;
        combat = true;
        combatUI = true;
        inventory = false;
    }
    ...
    public void Smokebomb()
    {
        ...
        if (smokeBomb >= 1)
        {
            ...
            combatEnemy.GetComponent<EnemyCombat>().player = null;
            combatEnemy = null;
            combat = false;
            playerTurn = false;
            combatUI = true;
    ...
    {
        Vector3 enemyLoc = -enemyCombat.transform.up;
        Vector3 enemyPos = enemyCombat.transform.position;

        Vector3 playerLoc = enemyLoc * 15 + enemyPos;
        playerLoc.x += enemyCombat.extraXDistance;
        playerLoc.y += 2.2f + enemyCombat.extraYDistance;
        playerLoc.z += enemyCombat.extraZDistance;
        this.gameObject.transform.position = playerLoc;

        enemyPos.y += 2.2f + enemyCombat.extraXDistance;
        Vector3 playerDirection = enemyPos - transform.position;
        Vector3 newDirection = Vector3.RotateTowards(transform.forward, playerDirection, 1, 0);
        transform.rotation = Quaternion.LookRotation(newDirection);

        cameraAdjust(2.2f, 1f, -3.2f, 0, -20, 0);
